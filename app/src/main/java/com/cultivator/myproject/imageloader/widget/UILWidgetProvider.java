 ...
 *******************************************************************************/
package com.cultivator.myproject.imageloader.widget;
...
import com.cultivator.myproject.imageloader.Constants;
import com.cultivator.myproject.imageloader.UILApplication;
...
public class UILWidgetProvider extends AppWidgetProvider {
	...
	static void updateAppWidget(Context context, final AppWidgetManager appWidgetManager, final int appWidgetId) {
		final RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.uil_widget);

		ImageSize minImageSize = new ImageSize(70, 70); // 70 - approximate size of ImageView in widget
		ImageLoader.getInstance().loadImage( Constants.IMAGES[0], minImageSize, optionsWithFakeDisplayer, new SimpleImageLoadingListener() {
			@Override
		...
		ImageLoader.getInstance().loadImage( Constants.IMAGES[1], minImageSize, optionsWithFakeDisplayer, new SimpleImageLoadingListener() {
			@Override
			public void onLoadingComplete(String imageUri, View view, Bitmap loadedImage) {
